# TP-9 Source: Vehicle Detection Details

**Historical source with incomplete option cells, curated for Git.** Examples identify the intended vehicle/road/motion assessments but do not supply a complete validated enum list. Live media/vehicle links are redacted; original SQL/comments remain reference only. See the [manual catalog](../../as-is/manual-processes.md).

**Purpose:** Protocol for adding additional details for “Confirmed” Halt Codes 12.27 (Vehicle), vehicle type, vehicle On or Off Road, and Vehicle Parked or Driving.
...
- **Autonomy state transition history **(Required only if Spark Engagement not Obvious)
    ...
    - Satellite map, used to visualization the position of the tractor 
