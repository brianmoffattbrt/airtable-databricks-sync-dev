...
The workspace checkout is preserved. The GitHub repository was renamed to brianmoffattbrt/triage-workflows under the same owner, retaining PUBLIC visibility; the earlier private-transfer proposal was superseded. No private-authentication transition was introduced. The stored old URL/checkout is not automatically rewritten, and future refresh/access must be checked separately. No Databricks pull/re-link/rename, credential change, validation Git folder, or notebook/job execution is included in setup. One preserved checkout is not a second GitHub repository.
